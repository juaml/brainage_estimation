...
def read_sub_data(phenotype_file, mask_dir, smooth_fwhm, resample_size):
    ...
    smooth_fwhm: Smooth images by applying a Gaussian filter with given FWHM in mm
    resampling_size: Resample image to given voxel size
    output:
    data_resampled: pandas dataframe of features (no. of subjects by no. of features)
    """
    ...
    for index, row in phenotype.iterrows(): # iterate over each row
        ...
        if os.path.exists(sub_file):
            ...
            sub_img = image.smooth_img(sub_img, smooth_fwhm) # smooth the image with 4 mm FWHM

            print('Perform resampling')
            # trying to match Gaser
            mask_img_rs = npr.resample_to_output(mask_img, [resample_size] * len(mask_img.shape), order=1) # resample mask
            print("mask affine after resampling\n", mask_img_rs.affine, mask_img_rs.shape)

            sub_img_rs = image.resample_to_img(sub_img, mask_img_rs, interpolation="linear") # resample subject
            print("sub affine after resampling\n", sub_img_rs.affine, sub_img_rs.shape)

            binary_mask_img_rs = binarize_3d(mask_img_rs, 0.5) # binarize the mask
            mask_rs = binary_mask_img_rs.get_fdata().astype(bool)

            sub_data_rs = sub_img_rs.get_fdata()[mask_rs] # extract voxel using the binarized mask
